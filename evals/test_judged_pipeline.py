"""L4+ evals with LLM-as-a-Judge scoring.

Runs the agent pipeline (planner → bootstrapper → worker → reviewer)
then scores outcomes with both static checks and LLM judge.
"""
...
class TestL4JudgedPipeline:
    """Full pipeline with static + LLM judge scoring."""
    ...
    def test_full_pipeline_judged(self, case):
        ...
        logger.info("Spec: %s", json.dumps(spec, indent=2)[:500])
