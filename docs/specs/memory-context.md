...
## 8. Стратегия поиска информации (RAG)

Семантический поиск фактов -- ключевой механизм контекстуализации агентов. Ниже описаны решения по каждому этапу RAG-пайплайна.

### 8.1 Выбор embedding-модели

| Критерий | Решение | Обоснование |
|---|---|---|
| Модель | `all-MiniLM-L6-v2` | Компактная (80MB), быстрая на CPU (~1ms/embedding), качество достаточно для коротких фактов |
| Размерность | 384 | Баланс между качеством поиска и размером индекса |
| Inference | CPU-only, локально | GPU занят LLM; embedding — лёгкая операция, CPU достаточно |
| Библиотека | `sentence-transformers` (Python) / `ort` (Rust через ONNX Runtime) | Python для агентов, Rust для оркестратора |
| Нормализация | L2 → cosine similarity | pgvector `vector_cosine_ops` ожидает ненормализованные вектора |

**Альтернативы (рассмотрены и отклонены):**

| Модель | Размерность | Причина отклонения |
|---|---|---|
| `text-embedding-3-small` (OpenAI) | 1536 | Внешний API, зависимость от сети, стоимость |
| `e5-large-v2` | 1024 | Слишком тяжёлая для CPU (~20ms/embedding), избыточное качество для коротких текстов |
| `bge-small-en-v1.5` | 384 | Сопоставимая по качеству, но `all-MiniLM-L6-v2` лучше документирована и протестирована |

### 8.2 Стратегия чанкинга

Факты в системе Finit — **атомарные единицы** (рекомендация: < 200 токенов), поэтому классический чанкинг документов неприменим. Вместо этого используется **structured chunking** на этапе записи:

```
Агент обнаруживает информацию
    ↓
Оформляет как факт (атомарное утверждение, < 200 токенов)
    ↓
Добавляет теги для фильтрации: ["dependency", "go", "chi"]
    ↓
save_fact(scope, content, tags)
    ↓
Embedding вычисляется и сохраняется в pgvector
```

**Правила формирования фактов (инструкция агентам):**

| Правило | Пример |
|---|---|
| Один факт = одно утверждение | "chi v5 используется как HTTP-роутер" (не "chi v5 используется, а обработчики в handler.go") |
| Включать контекст привязки | "В файле `handler.go`: 12 эндпоинтов, последний добавлен в iter-2" |
| Теги для фильтрации | `["file:handler.go", "endpoint", "iteration:2"]` |
| Избегать дубликатов | Перед `save_fact` агент вызывает `search_facts(content, limit=1)` и проверяет cosine similarity > 0.9 |

### 8.3 Стратегия поиска (retrieval)

Двухэтапный поиск: фильтрация по scope → семантический поиск по embedding.

```sql
-- Этап 1: фильтрация по scope
SELECT id, content, 1 - (embedding <=> $query_embedding) AS similarity
FROM memory_facts
WHERE scope_type = $scope_type
  AND (scope_id = $scope_id OR scope_type = 'global')
  AND ($tags IS NULL OR tags && $tags)  -- пересечение тегов
...
|---|---|---|
| `limit` | 10 | Максимум фактов в ответе |
| `min_similarity` | 0.5 | Порог релевантности (cosine) |
| `scope_priority` | task > workspace > global | Факты из узкого scope приоритетнее |
| `tag_filter` | опционально | Фильтрация по тегам до семантического поиска |

### 8.4 Reranking

Для коротких атомарных фактов (< 200 токенов) полноценный cross-encoder reranking избыточен. Вместо этого применяется **lightweight reranking** на стороне оркестратора:

```
1. Семантический поиск → top-K фактов (K = limit * 2)
2. Score boost по scope: task-scope × 1.2, workspace-scope × 1.0, global × 0.8
3. Score boost по recency: created_at < 1h × 1.1, < 24h × 1.0, > 7d × 0.9
4. Пересортировка по composite score
5. Возврат top-limit фактов
```

**Composite score:**

```
composite = cosine_similarity × scope_weight × recency_weight
```

Это обеспечивает приоритет свежих и контекстно-близких фактов без дополнительных моделей.

### 8.5 Индексация pgvector

| Параметр | Значение | Обоснование |
|---|---|---|
| Тип индекса | IVFFlat | Простой, предсказуемый, достаточный при < 100K фактов |
| `lists` | 100 | ~√N для N ≈ 10K фактов |
| `probes` | 10 | 10% lists — баланс recall/speed |
| Оператор | `vector_cosine_ops` | Cosine similarity для нормализованных embeddings |

При росте до > 100K фактов рекомендуется миграция на HNSW-индекс (`vector_cosine_ops`, `m=16`, `ef_construction=64`).
